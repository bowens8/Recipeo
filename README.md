...
The one thing that's genuinely not possible to auto-combine: if the *same ingredient* is
measured by weight in one recipe and by volume in another (e.g. one recipe says "2 cups
flour", another says "150 g flour"), there's no universal conversion between weight and
volume — it depends on the ingredient's density, which the app doesn't ask you to
provide. Those show up as a separate, clearly flagged line on the shopping list instead
of being silently combined incorrectly. In practice this is rare — pick one style
(weight or volume) per ingredient across your recipes and it never comes up. "each"
(whole items, like eggs) never mixes with a measurement either.
