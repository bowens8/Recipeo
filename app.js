...
// (volume<->volume or weight<->weight). Returns null if the units aren't
// in the same family — there's no manual conversion factor to fall back on,
// by design, so those amounts are just kept separate.
function convertQty(qty, fromUnit, toUnit){
  if (fromUnit === toUnit) return qty;
  const catFrom = unitCategory(fromUnit), catTo = unitCategory(toUnit);
  if (catFrom !== catTo || catFrom === 'count' || catFrom === 'unknown') return null;
...
function recipeCaloriesTotal(recipe){
  return (recipe.ingredients || []).reduce((sum, ri) => {
    ...
    const qtyInCanonicalUnit = convertQty(Number(ri.qty)||0, rowUnit, ing.unit);
...
function renderShoppingList(){
  ...
  Object.values(state.mealPlan).forEach(m => {
    ...
    (recipe.ingredients||[]).forEach(ri => {
      ...
      const rowCategory = unitCategory(rowUnit);
      const ingCategory = unitCategory(ing.unit);
      if (rowCategory === ingCategory && rowCategory !== 'unknown'){
        neededBase[ri.ingredientId] = (neededBase[ri.ingredientId]||0) + toBaseUnit(rawQty, rowUnit);
      } else {
        // e.g. this ingredient's reference unit is a weight but this recipe measured it by volume
  ...
  const warnHtml = warnRows.map(w => {
    ...
    return `<div class="shop-item warn" data-ing="${w.ingredientId}">
      ...
      <span class="s-price-block">
        ...
        <span class="s-warn-note">measured differently in this recipe (${UNIT_LABEL[w.unit]} vs. this ingredient's usual ${UNIT_LABEL[ing.unit]}) — shown separately</span>
      </span>
    </div>`;
  }).join('');
